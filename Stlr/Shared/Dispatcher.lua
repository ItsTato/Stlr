...
local HISTORY_SETTING_NAME = "StlrCommandHistory";
...
function Dispatcher:Evaluate(text: string, executor: Player, allowIncompleteArguments: boolean?, data: any?)
	if RunService:IsClient() == true and executor ~= Players.LocalPlayer then
		error("[Stlr] Can't evaluate a command that isn't sent by the local player.");
	end;
...
function Dispatcher:EvaluateAndRun(
	...
	if not ok then
		warn((`[Stlr] Error occurred while evaluating command string %q\n{tostring(out)}`):format(text));
	end;
...
function Dispatcher:Send(text: string, data: any?)
	if RunService:IsClient() == false then
		error("[Stlr] Dispatcher:Send can only be called from the client.");
	end;

	return self.Stlr.RemoteFunction:InvokeServer(text, {
		Data = data,
	});
end;
...
function Dispatcher:Run(...): string
	if not Players.LocalPlayer then
		error("[Stlr] Dispatcher:Run can only be called from the client.");
	end;
...
function Dispatcher:PushHistory(text: string)
	assert(RunService:IsClient(), "[Stlr] PushHistory may only be used from the client.");
...
function Dispatcher:GetHistory(): { string }
	assert(RunService:IsClient(), "[Stlr] GetHistory may only be used from the client.");

	return TeleportService:GetTeleportSetting(HISTORY_SETTING_NAME) or {};
end;

return function(stlr)
	Dispatcher.Stlr = stlr;
	Dispatcher.Registry = stlr.Registry;
