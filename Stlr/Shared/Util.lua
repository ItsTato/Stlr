...
--[=[
	...
	Stlr utilities module.

	:::info Beta
	This page is incomplete and some functions are missing.
	:::
]=]
local Util = {};
...
function Util.MakeFuzzyFinder(setOrContainer: any): (string, boolean?, boolean?) -> string
	...
	elseif typeof(setOrContainer) == "table" then
		...
		elseif setOrContainer[1] ~= nil then
			error("[Stlr] MakeFuzzyFinder only accepts tables of instances or strings.");
		else
			names = {};
		end;
	else
		error("[Stlr] MakeFuzzyFinder only accepts a table, Enum, or Instance.");
	end;
...
local function encodeControlChars(text)
	return (
		text:gsub("\\\\", "___!STLR_ESCAPE!___")
			:gsub('\\"', "___!STLR_QUOTE!___")
			:gsub("\\'", "___!STLR_SQUOTE!___")
			:gsub("\\\n", "___!STLR_NL!___")
	);
end

local function decodeControlChars(text)
	return (text:gsub("___!STLR_ESCAPE!___", "\\"):gsub("___!STLR_QUOTE!___", '"'):gsub("___!STLR_NL!___", "\n"));
end;
...
local function encodeCommandEscape(text)
	return (text:gsub("\\%$", "___!STLR_DOLLAR!___"));
end;
...
function Util.MakeSequenceType(options)
	...
	assert(options.Parse ~= nil or options.Constructor ~= nil, "[Stlr] MakeSequenceType: Must provide one of: Constructor, Parse");
