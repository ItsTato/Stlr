...
function Command.new(options)
	local self = {
		...
		Stlr = options.Dispatcher.Stlr, -- A quick reference to Stlr for command context
...
function Command:Parse(allowIncompleteArguments)
	...
	for i, definition in ipairs(self.ArgumentDefinitions) do
		...
		if required and hadOptional then
			error(("[Stlr] Command %q: Required arguments cannot occur after optional arguments."):format(self.Name));
...
function Command:Run()
	if self._Validated == nil then
		error("[Stlr] Command must be validated before running.");
	end;
	...
	if self.Response == nil then
		...
		elseif IsServer then -- Uh oh, we're already on the server and there's no Run function.
			if self.Object.ClientRun then
				warn(`[Stlr] {self.Name} command fell back to the server because ClientRun returned nil, but there is no server implementation! Either return a string from ClientRun, or create a server implementation for this command.`);
			else
				warn(`[Stlr] {self.Name} command has no implementation!`);
			end;
...
function Command:SendEvent(player, event, ...)
	assert(typeof(player) == "Instance" and player:IsA("Player"), "[Stlr] Argument #1 must be a Player");
	assert(type(event) == "string", "[Stlr] Argument #2 must be a string");

	if IsServer then
		self.Dispatcher.Stlr.RemoteEvent:FireClient(player, event, ...);
	elseif self.Dispatcher.Stlr.Events[event] then
		assert(player == Players.LocalPlayer,"[Stlr] Event messages can only be sent to the local player on the client.");
		self.Dispatcher.Stlr.Events[event](...);
	end;
end;
...
function Command:BroadcastEvent(...)
	if not IsServer then
		error("[Stlr] Can't broadcast event messages from the client.", 2);
	end;

	self.Dispatcher.Stlr.RemoteEvent:FireAllClients(...);
end;
...
function Command:GetStore(...)
	return self.Dispatcher.Stlr.Registry:GetStore(...);
end;
