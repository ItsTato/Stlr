...
function Argument.new(command, argumentDefinition, value)
	...
	if type(argumentDefinition.Type) == "table" then
		...
		local parsedType, parsedRawValue, prefix =Util.ParsePrefixedUnionType(command.Stlr.Registry:GetTypeName(argumentDefinition.Type), value);
		...
		if self.Type == nil then
			error((`[Stlr] {self.Name or "none"} has an unregistered type %q`):format(parsedType or "<none>"));
		end;
	end;
...
function Argument:GetValue(): any
	...
	for i = 1, #self.TransformedValues do
		...
		if type(parsedValue) ~= "table" then
			error(`[Stlr] Listable types must return a table from Parse {self.Type.Name}`);
		end;
