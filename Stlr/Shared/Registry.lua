...
--[=[
	...
	The registry keeps track of all the commands and types that Stlr knows about.
]=]
...
--[=[
	@prop Stlr Stlr | StlrClient
	@within Registry
	@readonly
	A reference to Stlr. This may either be the server or client version of Stlr depending on where the code is running.
]=]
...
function Registry:RegisterType(name: string, typeObject)
	if not name or typeof(name) ~= "string" then
		error("[Stlr] Invalid type name provided: nil");
	end;

	if not name:find("^[%d%l]%w*$") then
		error(`[Stlr] Invalid type name provided: "{name}", type names must be alphanumeric and start with a lower-case letter or a digit.`);
	end;

	for key in pairs(typeObject) do
		if self.TypeMethods[key] == nil then
			error(`[Stlr] Unknown key/method in type "{name}": {key}`);
		end;
	end;

	if self.Types[name] ~= nil then
		error(`[Stlr] Type {name} has already been registered.`);
	end;
...
function Registry:RegisterTypeAlias(name: string, alias: string)
	assert(self.TypeAliases[name] == nil, `[Stlr] Type alias {alias} already exists!`);
...
function Registry:RegisterTypesIn(container: Instance)
	for _, object in pairs(container:GetChildren()) do
		if object:IsA("ModuleScript") then
			object.Parent = self.Stlr.ReplicatedRoot.Types;
...
function Registry:RegisterCommandObject(commandObject)
	for key in pairs(commandObject) do
		if self.CommandMethods[key] == nil then
			error(`[Stlr] Unknown key/method in command "{commandObject.Name or "unknown command"}": {key}`);
		end;
	end;
			...
			if type(arg) == "table" then
				for key in pairs(arg) do
					if self.CommandArgProps[key] == nil then
						error(`[Stlr] Unknown property in command "{commandObject.Name or "unknown"}" argument #{i}: {key}`);
					end;
				end;
			end;
		end;
...
function Registry:RegisterCommand(
	...
	assert(typeof(commandObject) == "table",`[Stlr] Invalid return value from command script "{commandScript.Name}" (CommandDefinition expected, got {typeof(commandObject)})`);

	if commandServerScript then
		assert(RunService:IsServer(), "[Stlr] The commandServerScript parameter is not valid for client usage.");
	...
	commandScript.Parent = self.Stlr.ReplicatedRoot.Commands;
end;
...
function Registry:RegisterCommandsIn(container: Instance, filter: ((any) -> boolean)?)
	...
	for skippedScript in pairs(skippedServerScripts) do
		if not usedServerScripts[skippedScript] then
			warn(`[Stlr] Command script {skippedScript.Name} was skipped because it has 'Server' in its name, and has no equivalent shared script.`);
		end;
	end;
end;
...
function Registry:RegisterDefaultCommands(arrayOrFunc: { string } | (any) -> boolean | nil)
	assert(RunService:IsServer(), "[Stlr] RegisterDefaultCommands cannot be called from the client.");

	local dictionary = if type(arrayOrFunc) == "table" then Util.MakeDictionary(arrayOrFunc) else nil;

	self:RegisterCommandsIn(self.Stlr.DefaultCommandsFolder, dictionary ~= nil and function(command)
...
function Registry:RegisterHook(hookName: string, callback: (any) -> string?, priority: number)
	if not self.Hooks[hookName] then
		error(("[Stlr] Invalid hook name: %q"):format(hookName), 2);
	end;
...
function Registry:FlushAutoExecBuffer()
	for _, commandGroup in ipairs(self.AutoExecBuffer) do
		for _, command in ipairs(commandGroup) do
			self.Stlr.Dispatcher:EvaluateAndRun(command);
		end;
	end;
