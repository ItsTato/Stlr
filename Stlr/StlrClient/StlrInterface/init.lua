...
return function(Stlr)
	local Util = Stlr.Util;

	local Window = require(script:WaitForChild("Window"));
	Window.Stlr = Stlr;

	local AutoComplete = require(script:WaitForChild("AutoComplete"))(Stlr);
	...
	function Window.ProcessEntry(text);
		...
		Window:AddLine(Stlr.Dispatcher:EvaluateAndRun(text, Player, {
			IsHuman = true,
		}));
	end;
	...
	function Window.OnTextChanged(text)
		local command = Stlr.Dispatcher:Evaluate(text, Player, true);
		...
		elseif commandText and #arguments == 0 then
			...
			local exactCommand = Stlr.Registry:GetCommand(commandText);
			local exactMatch;
			...
			for _, cmd in pairs(Stlr.Registry:GetCommandNames()) do
				if
					commandText:lower() == cmd:lower():sub(1, #commandText)
					and (exactMatch == nil or exactMatch[1] ~= commandText)
				then
					local commandObject = Stlr.Registry:GetCommand(cmd);
					acItems[#acItems + 1] = {
						commandText,
						cmd,
						options = {
