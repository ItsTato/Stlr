...
local Gui = Player:WaitForChild("PlayerGui"):WaitForChild("Stlr"):WaitForChild("Frame");
...
function Window:UpdateLabel()
	Entry.TextLabel.Text = Player.Name .. "@" .. self.Stlr.PlaceName .. "$";
end;
...
function Window:AddLine(text, options)
	...
	local str = self.Stlr.Util.EmulateTabstops(text or "nil", 8);
...
function Window:SetVisible(visible)
	...
	if visible then
		...
		if self.Stlr.ActivationUnlocksMouse then
...
function Window:TraverseHistory(delta)
	local history = self.Stlr.Dispatcher:GetHistory();
...
function Window:BeginInput(input, gameProcessed)
	...
	if self.Stlr.ActivationKeys[input.KeyCode] then -- Activate the command bar
		if self.Stlr.MashToEnable and not self.Stlr.Enabled then
			if tick() - lastPressTime < 1 then
				if pressCount >= 5 then
					return self.Stlr:SetEnabled(true);
				else
	...
	if self.Stlr.Enabled == false or not self:IsVisible() then
		if self:IsVisible() then
			self:Hide();
		end;
	...
	if self.Stlr.HideOnLostFocus and table.find(MOUSE_TOUCH_ENUM, input.UserInputType) then
